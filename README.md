...
- 편집: 메일 제목, 브리핑 메모, 마무리 문구 직접 입력
- 파일 저장: 전체 결과 또는 선택 항목 기준으로 HTML 파일과 Markdown 파일을 각각 저장 가능
- 이메일 첨부: 발송 메일 본문과 함께 같은 내용을 HTML, Markdown 파일로도 첨부
...
- 구독 템플릿 실행: 저장된 템플릿으로 즉시 이메일 발송하거나 Markdown 저장 가능
...
- 내보내기: 선택 항목/전체 결과 기준 HTML 이메일 미리보기, HTML 저장, Markdown 다운로드
- 발송: 다중 이메일 수신자 대상 SMTP 발송
...
SMTP 발송을 사용하려면 `.env`에 `NEWSLETTER_SMTP_*` 환경 변수를 설정하세요. 예시는 [.env.example](.env.example)를 참조하면 됩니다.
...
`/newsletter` 화면 상단에는 현재 실행 환경, 열린국회 API 키 준비 여부, 저장소 방식, SMTP 준비 여부, cron 보호 설정이 함께 표시되고, 누락된 환경 변수 이름과 다음 조치도 바로 확인할 수 있습니다.

같은 화면의 `Vercel 환경 변수 보기` 버튼으로 복붙 가능한 env 템플릿을 열고, 바로 복사하거나 파일로 저장할 수도 있습니다. 이 템플릿에는 현재 배포 주소 기준 `newsletter`, `health`, `cron` URL과 `curl` 예시도 함께 들어 있습니다.

또한 `GitHub Actions cron 보기` 버튼으로 현재 배포 주소가 이미 들어간 외부 cron 워크플로 YAML도 바로 열고 저장할 수 있습니다.

`Vercel cron 설정 보기` 버튼으로는 `vercel.json`에 붙여 넣을 `crons` 스니펫 예시도 바로 확인할 수 있습니다.
